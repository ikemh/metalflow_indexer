...
const PYTHON = process.env.INDEXER_PYTHON || DEFAULT_PYTHON;
const PYTHON_ARGS_PREFIX = (process.env.INDEXER_PYTHON_ARGS || "")
  .split(" ")
  .map((s) => s.trim())
  .filter(Boolean);
...
function spawnThumbOnce(fileId, absoluteDxfPath) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      proc = spawn(
        PYTHON,
        [...PYTHON_ARGS_PREFIX, DXF_THUMB, absoluteDxfPath, String(THUMB_SIZE), outPath],
        { windowsHide: true },
      );
    } catch (err) {
      finish({
