...
function buildExcludedSet(excludedFolders) {
  const set = new Set();
...
/**
 * True se o customerFolder ou qualquer pasta intermediária do relativePath
 * estiver na lista de exclusões (compara em forma normalizada).
 * O último segmento (nome do arquivo) é ignorado.
 */
function pathHasExcludedSegment(customerFolder, relativePath, excludedSet) {
  if (!excludedSet || excludedSet.size === 0) return false;

  const normCust = normalizeExclusionName(customerFolder);
  if (normCust && excludedSet.has(normCust)) return true;

  const segments = relativePath.split("/").filter(Boolean);
  for (let i = 0; i < segments.length - 1; i++) {
    const n = normalizeExclusionName(segments[i]);
    if (n && excludedSet.has(n)) return true;
  }
  return false;
}
...
function createWatcher({ config, onCustomerChanged, log }) {
  ...
  const excludedSet = buildExcludedSet(config.excludedFolders);
  ...
  function scheduleSync(eventType, file) {
    ...
    if (pathHasExcludedSegment(customerFolder, relativePath, excludedSet)) {
      return;
    }
  ...
  function start() {
    ...
    watcher = chokidar.watch(paths, {
      ...
      ignored: (p, stats) => {
        // Diretórios sempre passam (até descobrir se são excluídos pelo nome).
        if (stats && stats.isDirectory && stats.isDirectory()) {
          // Se o nome do diretório bate com excludedFolders, ignora.
          const base = path.basename(p);
          const norm = normalizeExclusionName(base);
          if (norm && excludedSet.has(norm)) return true;
          return false;
        }
