...
// Defaults usados quando nenhum resolver está disponível (fallback puro).
// Os resolvers de config.js são a fonte primária.
const DEFAULT_USE_POLLING = process.platform === "win32";
const DEFAULT_INTERVAL_MS = 15000;
const DEFAULT_BINARY_INTERVAL_MS = 30000;
...
function createWatcher({ config, onCustomerChanged, log }) {
  ...
  function start() {
    ...
    // Use resolvers from config for watcher tuning parameters.
    const usePolling = resolveWatchUsePolling(config);
    const interval = resolveWatchIntervalMs(config);
    const binaryInterval = resolveWatchBinaryIntervalMs(config);
    const stabilityThreshold = resolveWatchStabilityThresholdMs(config);
    const pollInterval = resolveWatchPollIntervalMs(config);
